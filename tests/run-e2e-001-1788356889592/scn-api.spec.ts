...
test.describe('SCN', () => {
  ...
  test('SCN-001: Register new user via API with valid payload', async ({ request }) => {
    ...
    // Step 1: Send POST request to /api/register with valid payload
    ...
    expect(userBody.email).toBe(uniqueEmail);
    expect(userBody.name).toBe(payload.name);
  });
  ...
  test('SCN-003: Filter products by category via API', async ({ request }) => {
    ...
    // Step 3: Send GET request with a non-existent category
  ...
  test('SCN-004: Checkout API processes order and payment successfully', async ({ request }) => {
    const checkoutPayload = {
      ...
      paymentToken: 'sandbox_payment_token_valid'
    };

    // Step 1: Send POST request to /api/checkout with valid cart, shipping, and payment
    ...
    const orderResponse = await request.get(`${BASE_URL}/api/orders/${orderId}`);
  ...
  test('SCN-005: API rejects add-to-cart request for out-of-stock product', async ({ request }) => {
    ...
    const cartItems = cartBody.items || cartBody;
    expect(Array.isArray(cartItems)).toBeTruthy();

    const outOfStockItemInCart = cartItems.find(
      (item: any) => item.productId === outOfStockProductId
    );
    expect(outOfStockItemInCart).toBeUndefined();
  });
